...
**Goal:** Make Search Everywhere (Conch's command palette) always open horizontally centered on the Conch workbench window with its vertical center at 25% of the window height — the "center of the upper half" look users expect from a command palette. Ignore any previously-saved popup location.
...
## Component: `ConchSearchEverywhereLocationPinner`

New file at `core/src/com/conch/core/palette/ConchSearchEverywhereLocationPinner.java`.
...
- **Matches any `SearchEverywhereAction` subclass.** The `instanceof` check catches both the base action (bound to `Cmd+Shift+P` in Conch as `Conch.CommandPalette`) and any platform-provided subclasses like `SearchEverywhereRiderMainToolbarAction`. If we ever add a custom Conch subclass, it's covered automatically.
...
1. Launch Conch. Open palette with `Cmd+Shift+P`. Expected: appears horizontally centered in the Conch window, vertical center at ~25% of window height.
...
4. Move the Conch window to a different monitor. Open palette. Expected: positioned relative to the Conch window on the new monitor, not frozen on the old one.
...
1. **View type persistence.** The platform's upper-half-center branch is gated on `viewType == SHORT`. If the user drag-resizes the popup tall enough to trip `FULL` view type, the platform persists that and subsequent opens take the fallback `showCenteredInCurrentWindow` path — which will still be centered on the Conch window but not at the upper-half vertical offset. Deferred fix: also reset view type, either by adding it to this listener or by a supplemental service override. Not addressed in this spec unless the caveat actually bites in practice.

2. **Coverage.** The listener only fires for action-system invocations. All Conch entry points for Search Everywhere go through `AnAction` today (`Conch.CommandPalette` action, the Rider-style toolbar action if it's ever re-enabled). A hypothetical non-action entry point that directly calls `SearchEverywhereManager.show()` would bypass the listener. Not a current concern.
...
**In scope:**
- `ConchSearchEverywhereLocationPinner.java` (new file)
...
- Changes to `ConchSearchEverywhereCustomizer` or `ConchTabsCustomizationStrategy`
