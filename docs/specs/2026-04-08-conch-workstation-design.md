# Conch Workstation — Design Specification

> **Build approach updated 2026-04-10.** This spec was originally written assuming Conch would be a standalone Gradle project consuming the IntelliJ Platform as a Maven dependency. The implementation took a different route: Conch is now built **inside** the `intellij-community` source tree using **Bazel**, the same build system JetBrains uses internally. The Conch source lives in this repo (`conch_workbench`) and is symlinked into `intellij-community/conch/` so Bazel can find it. See `README.md` in this repo for the layout.
>
...
> - Module/package layout described below is conceptually correct, but the actual on-disk paths are under `conch_workbench/` (this repo) rather than the original `conch_2/` Gradle layout.
> - Everything else — extension points, terminal/workspace model, plugin responsibilities, security model, file layout under `~/.config/conch/` — is still authoritative.
>
> Implementation plans live alongside this spec under `docs/plans/`. Each plan notes any further divergence from this spec.

## Overview

Conch is a terminal-driven workstation built as an IntelliJ Platform product. Where IntelliJ IDEA is an IDE with code editors as the primary viewport, Conch replaces editors with JediTerm terminal instances. It ships with bundled plugins for SSH host management, SFTP file browsing, an encrypted credential vault, and SSH tunnel management. Third-party plugins extend the workstation through the same extension point system used by the bundled plugins.
...
- The developer (Dustin) must be able to understand and debug every line of Conch-specific code
...
| Plugin SDK | Published Maven/Gradle artifact (`conch-plugin-sdk`) |

### 1.2 Stripped IntelliJ Platform

Conch excludes all developer-focused plugins and infrastructure:
...
**Conch-defined extension points** (defined by the core module):
...
Workspace state lives in `~/.config/conch/workspaces/`. The default workspace is `default.json`.
...
Plugins are standard IntelliJ Platform plugins: a JAR (or set of JARs) with a `plugin.xml` descriptor. Plugin authors depend on `conch-plugin-sdk` (published Maven/Gradle artifact) which contains Conch-specific interfaces and extension points.

### 4.2 Plugin Development Workflow

1. Create a Gradle project with `org.jetbrains.intellij` plugin
2. Add `conch-plugin-sdk` as a dependency
3. Implement interfaces, register in `plugin.xml`
4. Build JAR, drop into `~/.config/conch/plugins/` or install via plugin manager
...
- **Host key verification**: first-connect prompt with SHA256 fingerprint, stored in `~/.config/conch/known_hosts`, mismatch warning (potential MITM)
...
- **Persistence**: `~/.config/conch/servers.json`, atomic writes
...
- **Encryption**: AES-256-GCM with Argon2id key derivation (m=65536, t=3, p=4). Vault file always encrypted on disk at `~/.config/conch/vault.enc`
...
- Known hosts stored in `~/.config/conch/known_hosts`
- Host key mismatch warns user (potential MITM)
- Credential lifetime: read from vault, passed to SSH library, zeroed. Plaintext credentials do not persist in Conch code.
...
|---|---|---|
| `~/Library/Application Support/Conch/` | IntelliJ platform settings (keymap, appearance, UI state) | Platform |
| `~/.config/conch/workspaces/` | Saved workspace state files | Core |
| `~/.config/conch/servers.json` | SSH host/folder/tunnel configuration | SSH Host Manager plugin |
| `~/.config/conch/known_hosts` | SSH host key store | SSH Host Manager plugin |
| `~/.config/conch/vault.enc` | Encrypted credential vault | Vault plugin |
| `~/.config/conch/plugins/` | User-installed plugin JARs | Platform |

Settings managed through the Settings UI use IntelliJ's built-in persistence (XML). Conch-specific data (SSH hosts, vault, workspaces) uses `~/.config/conch/` with atomic writes (temp file + rename).
...
- **conch_remote** (`~/projects/rusty_conch_2/crates/conch_remote/`): SSH connection management, SFTP operations, tunnel manager, file transfers, known hosts. The `RemoteCallbacks` trait pattern maps to Conch's `CredentialProvider` and event-based prompting model.
- **conch_vault** (`~/projects/rusty_conch_2/crates/conch_vault/`): Vault encryption (AES-GCM + Argon2), account CRUD, SSH key generation, lock manager with inactivity timeout, in-memory SSH agent. The data model (`VaultAccount`, `AuthMethod`, `GeneratedKeyEntry`) and encryption format should be replicated in Java.
- **conch plugin system** (`~/projects/conch_2/crates/conch_plugin/`): HostApi trait, plugin bus (pub/sub + query routing), widget system, Lua/Java runtimes. The `HostApi` trait's surface area informed the Conch extension point design.
