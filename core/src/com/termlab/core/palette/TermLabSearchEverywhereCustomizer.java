 ...
 * TermLab's Command Palette (rebranded Search Everywhere) never surfaces files,
 * classes, symbols, text matches, run configurations, top hits, calculator
 * results, or YAML keys.
 *
 * <p>{@link TermLabTabsCustomizationStrategy} separately limits which contributors
 ...
 *   <li>{@code TermLabTerminals} (our own {@link TerminalPaletteContributor}) —
 ...
 * identity.
 */
public final class TermLabSearchEverywhereCustomizer implements AppLifecycleListener {

    private static final Logger LOG = Logger.getInstance(TermLabSearchEverywhereCustomizer.class);
    ...
    private static final Set<String> UNWANTED_FACTORIES = Set.of(
        ...
        // Git (not in TermLab's product, but listed for completeness if it gets added)
    ...
    public void appFrameCreated(@NotNull List<String> commandLineArgs) {
        ...
        if (removed) {
            LOG.info("TermLab: stripped built-in Search Everywhere contributors "
