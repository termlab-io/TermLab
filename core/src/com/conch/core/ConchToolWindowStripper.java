 ...
 * {@link ConchToolWindowCustomizer}, but for tool windows that aren't
 ...
 *   <li><b>Debug</b> — similar to Run, via {@code DebugExecutor}. Conch
 ...
 */
public final class ConchToolWindowStripper implements ToolWindowManagerListener {

    private static final Logger LOG = Logger.getInstance(ConchToolWindowStripper.class);
    ...
    public ConchToolWindowStripper(@NotNull Project project) {
    ...
    private void stripToolWindow(@NotNull String id) {
        try {
            ...
            LOG.info("Conch: stripped dynamic tool window '" + id + "'");
        } catch (Exception e) {
            ...
            LOG.warn("Conch: failed to strip tool window '" + id + "': " + e.getMessage());
        }
    }
}
