...
import com.conch.core.actions.NoopExecutorStubAction;
 ...
 *
 * <p>Conch's main frame is the terminal tool window — there is no
 ...
 * Configuration, etc.) because Conch inherits them from the
 ...
 * pattern as {@link ConchRefactoringStripper} takes for the
 * refactoring menu.
 *
 ...
 * on null. Conch uses Classic UI so most of those paths don't
 ...
 * groups).
 */
public final class ConchToolbarStripper implements AppLifecycleListener {

    private static final Logger LOG = Logger.getInstance(ConchToolbarStripper.class);
    ...
    public void appFrameCreated(@NotNull List<String> commandLineArgs) {
        ...
        for (String id : UNWANTED_ACTION_IDS) {
            ...
            removed++;
            LOG.info("Conch: unregistered toolbar action '" + id + "'");
        }
        ...
        for (String id : STUBBED_ACTION_IDS) {
            ...
            stubbed++;
            LOG.info("Conch: replaced '" + id + "' with NoopExecutorStubAction");
        }

        LOG.info("Conch: stripped " + removed + " toolbar action(s), stubbed "
