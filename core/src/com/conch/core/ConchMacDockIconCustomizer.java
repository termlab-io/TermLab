 ...
 * blurry icon. The {@code -Dapple.awt.application.icon=...conch.icns}
 ...
 * icon update.
 */
public final class ConchMacDockIconCustomizer implements AppLifecycleListener {

    private static final Logger LOG = Logger.getInstance(ConchMacDockIconCustomizer.class);
     ...
     */
    private static final String ICON_RESOURCE = "/conch_dock_icon.png";
    ...
    private static @org.jetbrains.annotations.Nullable BufferedImage loadIcon() {
        try (InputStream in = ConchMacDockIconCustomizer.class.getResourceAsStream(ICON_RESOURCE)) {
            if (in == null) {
                LOG.warn("Conch: dock icon resource not found at " + ICON_RESOURCE);
                return null;
            }
            BufferedImage image = ImageIO.read(in);
            if (image == null) {
                LOG.warn("Conch: could not decode dock icon at " + ICON_RESOURCE);
                return null;
            }
            LOG.info("Conch: loaded dock icon " + image.getWidth() + "x" + image.getHeight());
            return image;
        } catch (Exception e) {
            LOG.warn("Conch: failed to load dock icon", e);
            return null;
        }
    }
    ...
    private static void applyDockIcon(@NotNull Image image) {
        try {
            ...
            LOG.warn("Conch: could not set dock icon: " + e.getMessage());
        }
    }
}
