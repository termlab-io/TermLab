 ...
 * resolves to a path that does not exist in TermLab's layout
...
public final class TextMateBuiltinBundleEnabler implements ProjectActivity {
    ...
    public @Nullable Object execute(@NotNull Project project, @NotNull Continuation<? super Unit> continuation) {
        ...
        // ----------------------------------------------------------------
        LOG.warn("TERMLAB_EDITOR_DEBUG: startup — TermLab editor plugin loaded");

        String home = PathManager.getHomePath();
        LOG.warn("TERMLAB_EDITOR_DEBUG: PathManager.getHomePath=" + home);
        LOG.warn("TERMLAB_EDITOR_DEBUG: PathManager.getPluginsPath=" + PathManager.getPluginsPath());

        Path bundlesPath = Paths.get(home, "plugins", "textmate", "lib", "bundles");
        LOG.warn("TERMLAB_EDITOR_DEBUG: expected bundles path=" + bundlesPath + " exists=" + Files.exists(bundlesPath));
        ...
        try {
            ...
            LOG.warn("TERMLAB_EDITOR_DEBUG: TextMateService.class loaded from: " + svcClass.getProtectionDomain().getCodeSource());
            Object svc = ApplicationManager.getApplication().getService(svcClass);
            LOG.warn("TERMLAB_EDITOR_DEBUG: TextMateService instance=" + (svc == null ? "NULL" : svc.getClass().getName()));
        } catch (ClassNotFoundException e) {
            LOG.warn("TERMLAB_EDITOR_DEBUG: TextMateService class NOT on classpath");
        } catch (Throwable t) {
            LOG.warn("TERMLAB_EDITOR_DEBUG: TextMateService lookup threw: " + t.getMessage());
        }
        ...
        LOG.warn("TERMLAB_EDITOR_DEBUG: registered file type count BEFORE fix=" + typesBefore.length);
        int tmCountBefore = 0;
        for (FileType ft : typesBefore) {
            if (ft.getClass().getName().toLowerCase().contains("textmate")) {
                LOG.warn("TERMLAB_EDITOR_DEBUG: registered TextMate type (before): name=" + ft.getName()
                        + " class=" + ft.getClass().getName());
                tmCountBefore++;
            }
        }
        LOG.warn("TERMLAB_EDITOR_DEBUG: total TextMate-typed file types registered BEFORE fix=" + tmCountBefore);
        ...
        LOG.warn("TERMLAB_EDITOR_DEBUG: registered file type count AFTER fix=" + typesAfter.length
        ...
        for (FileType ft : typesAfter) {
            if (ft.getClass().getName().toLowerCase().contains("textmate")) {
                LOG.warn("TERMLAB_EDITOR_DEBUG: registered TextMate type (after): name=" + ft.getName()
                        + " class=" + ft.getClass().getName());
                tmCountAfter++;
            }
        }
        LOG.warn("TERMLAB_EDITOR_DEBUG: total TextMate-typed file types registered AFTER fix=" + tmCountAfter
    ...
    private static void enableBuiltinBundlesIfNeeded(Path bundlesPath) {
        if (!Files.isDirectory(bundlesPath)) {
            LOG.warn("TERMLAB_EDITOR_DEBUG: [enabler] bundles dir not found at " + bundlesPath + " — skipping");
            return;
        }
        ...
        try {
            ...
            LOG.warn("TERMLAB_EDITOR_DEBUG: [enabler] TextMateBuiltinBundlesSettings not on classpath — skipping", e);
            return;
        }

        if (builtinSettings == null) {
            LOG.warn("TERMLAB_EDITOR_DEBUG: [enabler] TextMateBuiltinBundlesSettings service is null (not registered?) — skipping");
            return;
        }
        ...
        try {
            ...
            LOG.warn("TERMLAB_EDITOR_DEBUG: [enabler] getBuiltinBundles() failed: " + t.getMessage(), t);
            return;
        }

        LOG.warn("TERMLAB_EDITOR_DEBUG: [enabler] existing builtinBundles.size=" + existingBundles.size());

        if (!existingBundles.isEmpty()) {
            // Already populated (e.g. pre-set by another init path) — just reload.
            LOG.warn("TERMLAB_EDITOR_DEBUG: [enabler] builtinBundles already populated; triggering reload");
        ...
        try {
            ...
            LOG.warn("TERMLAB_EDITOR_DEBUG: [enabler] TextMateBundleToLoad not accessible: " + t.getMessage(), t);
            return;
        }
            ...
            entries.filter(p -> Files.isDirectory(p) && !p.getFileName().toString().startsWith("."))
                   .forEach(bundleDir -> {
                       ...
                       try {
                           ...
                           LOG.warn("TERMLAB_EDITOR_DEBUG: [enabler] failed to construct TextMateBundleToLoad for "
                                   + path + ": " + t.getMessage());
                       }
                   });
        } catch (IOException e) {
            LOG.warn("TERMLAB_EDITOR_DEBUG: [enabler] failed to list bundles dir: " + e.getMessage(), e);
            return;
        }

        LOG.warn("TERMLAB_EDITOR_DEBUG: [enabler] found " + bundlesToLoad.size() + " bundles at " + bundlesPath);
        if (bundlesToLoad.isEmpty()) {
            LOG.warn("TERMLAB_EDITOR_DEBUG: [enabler] no bundles found — skipping");
            return;
        }
        ...
        try {
            ...
            LOG.warn("TERMLAB_EDITOR_DEBUG: [enabler] setBuiltinBundles() called with "
                    + bundlesToLoad.size() + " entries"
                    + " via " + setter.getDeclaringClass().getName() + ".setBuiltinBundles(List)");
        } catch (Throwable t) {
            LOG.warn("TERMLAB_EDITOR_DEBUG: [enabler] setBuiltinBundles() failed: " + t.getMessage(), t);
            return;
        }
        ...
        // our probes run.
        try {
            LOG.warn("TERMLAB_EDITOR_DEBUG: [enabler] flushing EDT to apply async extensionMapping update...");
            ApplicationManager.getApplication().invokeAndWait(() ->
                LOG.warn("TERMLAB_EDITOR_DEBUG: [enabler] EDT flush complete — extensionMapping should now be populated")
            );
        } catch (Throwable t) {
            LOG.warn("TERMLAB_EDITOR_DEBUG: [enabler] invokeAndWait flush threw: " + t.getMessage(), t);
        }
    }
    ...
    private static void reloadEnabledBundles() {
        try {
            ...
            Object svc = ApplicationManager.getApplication().getService(svcClass);
            if (svc == null) {
                LOG.warn("TERMLAB_EDITOR_DEBUG: [enabler] TextMateService instance is null — cannot reload");
                return;
            }
            Method reload = svcClass.getMethod("reloadEnabledBundles");
            LOG.warn("TERMLAB_EDITOR_DEBUG: [enabler] calling "
                    + reload.getDeclaringClass().getName() + ".reloadEnabledBundles()");
            reload.invoke(svc);
            LOG.warn("TERMLAB_EDITOR_DEBUG: [enabler] reloadEnabledBundles() returned"
                    + " (extensionMapping update is async — see EDT flush below)");
        } catch (Throwable t) {
            LOG.warn("TERMLAB_EDITOR_DEBUG: [enabler] reloadEnabledBundles() threw: "
    ...
    private static void probeTextMateDescriptors() {
        ...
        try {
            ...
            if (svc == null) {
                LOG.warn("TERMLAB_EDITOR_DEBUG: [probe] TextMateService is null — cannot probe");
                return;
            }
            getDescriptor = svcClass.getMethod("getLanguageDescriptorByFileName", CharSequence.class);
        } catch (Throwable t) {
            LOG.warn("TERMLAB_EDITOR_DEBUG: [probe] reflection setup failed: " + t.getMessage(), t);
            return;
        }
        ...
        try {
            ...
            if (mapping instanceof java.util.Map) {
                LOG.warn("TERMLAB_EDITOR_DEBUG: [probe] extensionMapping.size=" + ((java.util.Map<?,?>) mapping).size()
        ...
        for (String p : probes) {
            try {
                ...
                LOG.warn("TERMLAB_EDITOR_DEBUG: [probe] getLanguageDescriptorByFileName('" + p + "') -> "
                        + (descriptor == null ? "NULL (not recognised)" : descriptor.getClass().getSimpleName() + " (OK)"));
            } catch (Throwable t) {
                LOG.warn("TERMLAB_EDITOR_DEBUG: [probe] '" + p + "' threw: " + t.getMessage());
            }
        }
    }
}
