...
import com.termlab.ssh.ui.HostKeyPromptDialog;
 ...
 * {@code @TempDir}.
 */
public final class TermLabServerKeyVerifier implements ServerKeyVerifier {

    private static final Logger LOG = Logger.getInstance(TermLabServerKeyVerifier.class);
    ...
    public TermLabServerKeyVerifier() {
        this(HostPaths.knownHostsFile(), new SwingPromptUi());
    }

    /** Test constructor — explicit path and UI. */
    public TermLabServerKeyVerifier(@NotNull Path knownHostsPath, @NotNull PromptUi promptUi) {
    ...
    public boolean verifyServerKey(
        ...
        LOG.info("TermLab SSH: verifyServerKey entry"
        ...
        LOG.info("TermLab SSH: host key verify target=" + host + ":" + port + " source=" + target.source());

        KnownHostsFile.Match match;
        try {
            match = KnownHostsFile.match(knownHostsPath, host, port, serverKey);
        } catch (IOException e) {
            LOG.warn("TermLab: failed to read known_hosts at " + knownHostsPath, e);
            return false;
        }
        LOG.info("TermLab SSH: host key verify result target=" + host + ":" + port + " match=" + match);

        return switch (match) {
            case MATCH -> {
                LOG.info("TermLab SSH: host key MATCH, proceeding silently target=" + host + ":" + port);
                yield true;
            }
            case MISMATCH -> {
                LOG.warn("TermLab SSH: host key MISMATCH, refusing target=" + host + ":" + port);
    ...
    private boolean handleUnknown(String host, int port, PublicKey serverKey) {
        ...
        LOG.info("TermLab SSH: host key UNKNOWN, dispatching prompt target=" + hostSpec
        ...
        try {
            ...
            LOG.warn("TermLab SSH: host key prompt threw for target=" + hostSpec
                + " after " + ((System.nanoTime() - startNs) / 1_000_000) + "ms", t);
            return false;
        }
        long elapsedMs = (System.nanoTime() - startNs) / 1_000_000;
        LOG.info("TermLab SSH: host key prompt returned target=" + hostSpec
        ...
        return switch (decision) {
            case ACCEPT_AND_SAVE -> {
                try {
                    ...
                    LOG.info("TermLab SSH: host key appended to known_hosts target=" + hostSpec);
                } catch (IOException e) {
                    // Log and proceed — an I/O failure here doesn't compromise
                    // the current session; it just means next connect re-prompts.
                    LOG.warn("TermLab: failed to append " + host + ":" + port
    ...
    private static final class SwingPromptUi implements PromptUi {
        ...
        public @NotNull HostKeyPromptDialog.Decision prompt(
            ...
            LOG.info("TermLab SSH: SwingPromptUi.prompt enqueue target=" + hostSpec
                + " callerThread=" + Thread.currentThread().getName());

            ApplicationManager.getApplication().invokeAndWait(() -> {
                long dispatchMs = (System.nanoTime() - enqueueNs) / 1_000_000;
                LOG.info("TermLab SSH: SwingPromptUi.prompt EDT dispatch target=" + hostSpec
                ...
                try {
                    ...
                    LOG.info("TermLab SSH: SwingPromptUi.prompt showAndDecide returned target=" + hostSpec
                        + " decision=" + holder[0]);
                } catch (Throwable t) {
                    LOG.warn("TermLab SSH: SwingPromptUi.prompt dialog construction/show failed target="
                        + hostSpec, t);
            ...
            LOG.info("TermLab SSH: SwingPromptUi.prompt invokeAndWait returned target=" + hostSpec
        ...
        public void showMismatch(@NotNull String host, int port) {
            ApplicationManager.getApplication().invokeAndWait(() -> Messages.showErrorDialog(
                "The host key presented by " + host + ":" + port + " does not match the "
                    + "one TermLab has on file. Someone may be intercepting your connection "
