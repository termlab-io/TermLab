...
class ConchServerKeyVerifierProxyJumpTest {
    ...
    void verifyServerKey_usesTargetServerAttributeForProxyJumpSession(@TempDir Path tmp) throws Exception {
        ...
        ConchServerKeyVerifier.PromptUi promptUi = (ConchServerKeyVerifier.PromptUi) Proxy.newProxyInstance(
            ConchServerKeyVerifierProxyJumpTest.class.getClassLoader(),
            new Class<?>[]{ConchServerKeyVerifier.PromptUi.class},
            (proxy, method, args) -> {
                throw new AssertionError("Prompt UI should not be called for known-hosts MATCH: " + method.getName());
            });
        ConchServerKeyVerifier verifier = new ConchServerKeyVerifier(knownHosts, promptUi);
